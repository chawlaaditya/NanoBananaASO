# Quality Gates

Use this checklist before presenting the campaign as done.

## Story

- The first slide clearly sells the strongest user benefit.
- Every slide sells one idea in about one second.
- The slides feel like one campaign, not independent mockups.
- The sequence tells a coherent story from outcome to product proof to value.

## Consistency

- Slide 1 clearly establishes the family style.
- Every later slide visibly inherits that family style.
- Typography scale and tone feel consistent.
- Panel treatment and decorative language feel related.
- Accent colors and background logic do not randomly drift.
- Supporting imagery feels category-appropriate and coherent across the set.

## Layout

- Adjacent slides do not repeat the same phone placement.
- The phone or main focal object feels intentional and dominant where needed.
- Value slides without screenshots still feel part of the same campaign.
- Decorative elements support the message instead of filling empty space.
- The normalized export does not crop out any critical text or focal visuals.
- Screenshot-based slides are not reduced to text-over-screenshot billboards.

## Copy

- Headlines are readable at thumbnail size.
- Support lines are short and secondary.
- Text is not repeated in multiple chips without adding value.
- Claims are grounded in the real product.

## Common Failures

Revise the set if you see any of these:
- generic or random abstract backgrounds
- plain screenshot-in-a-mockup slides
- same layout repeated across multiple slides
- value slide that looks like “AI art” instead of a product ad
- too much copy on a single slide
- decorative chips that simply restate the support line
- headlines, chips, or key visuals clipped after normalization
- random people, places, or objects that do not clearly belong to the app story
- vibrant colors or photos that overpower the screenshot instead of supporting it
